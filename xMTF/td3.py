...
class TD3(object):
    ...
    def __init__(self, model_type, data_type, reward_type, stage, a_dim, s_dim, a_bound, lr_a=0.0001, lr_c=0.0002,
                 gamma=0.9, tau=0.01, memory_capacity=1000, batch_size=32, summary_writer=None):
        """
        ...
        W_init = tf.random_uniform_initializer(minval=-0.003, maxval=0.003,seed=99)
        b_init = tf.constant_initializer(0.1)

        # 建立actor网络，输入s，输出a
        def get_actor(input_state_shape, W_init=tf.random_uniform_initializer(minval=-0.003, maxval=0.003,seed=99), name=''):
            """
            Build actor network
        ...
        def get_critic(input_state_shape, input_action_shape, W_init=tf.random_uniform_initializer(minval=-0.003, maxval=0.003,seed=99), name=''):
            """
        ...
        self.critic_1 = get_critic([None, s_dim], [None, a_dim], W_init=tf.random_uniform_initializer(minval=-0.003, maxval=0.003,seed=100), name="_1")
        self.critic_2 = get_critic([None, s_dim], [None, a_dim], W_init=tf.random_uniform_initializer(minval=-0.003, maxval=0.003,seed=101), name="_2")
        self.actor.train()
        self.critic_1.train()
        self.critic_2.train()
        ...
        self.critic_1_target = get_critic([None, s_dim], [None, a_dim], name='_1_target')
        self.critic_2_target = get_critic([None, s_dim], [None, a_dim], name='_2_target')
        copy_para(self.critic_1, self.critic_1_target)
        copy_para(self.critic_2, self.critic_2_target)
        ...
        self.critic_1_opt = tf.optimizers.Adam(self.lr_c)
        self.critic_2_opt = tf.optimizers.Adam(self.lr_c)
    ...
    def ema_update(self):
        ...
        paras = self.actor.trainable_weights + self.critic_1.trainable_weights + self.critic_2.trainable_weights  # 获取要更新的参数包括actor和critic的
        self.ema.apply(paras)  # 主要是建立影子参数
        for i, j in zip(self.actor_target.trainable_weights + self.critic_1_target.trainable_weights + self.critic_2_target.trainable_weights, paras):
    ...
    def learn(self):
        ...
        for epoch in range(2):
            with tf.GradientTape() as tape1:
                with tf.GradientTape() as tape2:
                    ...
                    noise = tf.random.normal(tf.shape(a_), mean=0, stddev=0.01)
                    noise = tf.clip_by_value(noise, -0.1, 0.1)
                    a_ = tf.clip_by_value(a_ + noise, 0.0, 1.0)
                    q_1_ = self.critic_1_target([bs_, a_])
                    q_2_ = self.critic_2_target([bs_, a_])
                    y = br + self.gamma * tf.reduce_min(tf.concat([q_1_, q_2_], axis=-1), axis=-1)
                    q1 = self.critic_1([bs, ba])
                    q2 = self.critic_2([bs, ba])
                    td_error_1 = tf.losses.mean_squared_error(y, q1)
                    td_error_2 = tf.losses.mean_squared_error(y, q2)
                c1_grads = tape1.gradient(td_error_1, self.critic_1.trainable_weights)
                c2_grads = tape2.gradient(td_error_2, self.critic_2.trainable_weights)
                self.critic_1_opt.apply_gradients(zip(c1_grads, self.critic_1.trainable_weights))
                self.critic_2_opt.apply_gradients(zip(c2_grads, self.critic_2.trainable_weights))
                # print(f"\033[0;31;42m epoch:{epoch}, td_error {tf.reduce_mean(td_error_1)} {tf.reduce_mean(td_error_2)}\033[0m")
                self.td_error = tf.reduce_mean(td_error_1)
        # Actor：
        # Actor的目标就是获取最多Q值的。
        with tf.GradientTape() as tape:
            ...
            # a_loss = mse_loss
            self.a_loss = tf.reduce_mean(a_loss)
        a_grads = tape.gradient(a_loss, self.actor.trainable_weights)
        self.actor_opt.apply_gradients(zip(a_grads, self.actor.trainable_weights))
        # print(f"\033[0;31;42ma_loss {tf.reduce_mean(a_loss)}, {-tf.reduce_mean(q)}\033[0m")
    ...
    def store_transition(self, s, a, r, s_):
        ...
        #print(f"transition len:{len(transition)}, s len:{len(s)}, s_ len:{len(s_)}, a len:{len(a)}, r len:{len([r])}")
    ...
    def save_ckpt(self, mfc_mode, exit_mode, step_id, formula):
        ...
        tl.files.save_weights_to_hdf5(os.path.join(model_path, f'{self.stage}_critic_1.hdf5'), self.critic_1)
        tl.files.save_weights_to_hdf5(os.path.join(model_path, f'{self.stage}_critic_2.hdf5'), self.critic_2)
        tl.files.save_weights_to_hdf5(os.path.join(model_path, f'{self.stage}_critic_1_target.hdf5'), self.critic_1_target)
        tl.files.save_weights_to_hdf5(os.path.join(model_path, f'{self.stage}_critic_2_target.hdf5'), self.critic_2_target)
    ...
    def load_ckpt(self, mfc_mode, exit_mode, step_id, formula):
        ...
        tl.files.load_hdf5_to_weights_in_order(os.path.join(model_path, f'{self.stage}_actor_target.hdf5'),
                                               self.actor_target)
        tl.files.load_hdf5_to_weights_in_order(os.path.join(model_path, f'{self.stage}_critic_1.hdf5'), self.critic_1)
        tl.files.load_hdf5_to_weights_in_order(os.path.join(model_path, f'{self.stage}_critic_2.hdf5'), self.critic_2)
        tl.files.load_hdf5_to_weights_in_order(os.path.join(model_path, f'{self.stage}_critic_1_target.hdf5'),
                                               self.critic_1_target)
        tl.files.load_hdf5_to_weights_in_order(os.path.join(model_path, f'{self.stage}_critic_2_target.hdf5'),
